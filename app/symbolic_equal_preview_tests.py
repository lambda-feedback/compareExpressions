...
class TestPreviewFunction():
    ...
    @pytest.mark.parametrize("is_latex_value", [True, False])
    def test_latex_and_sympy_are_returned(self, is_latex_value):
        response = "x+1"
        params = Params(is_latex=is_latex_value)
        result = preview_function(response, params)
        assert "preview" in result.keys()
    ...
    def test_doesnt_simplify_latex_by_default(self):
        ...
        assert preview["sympy"] == "(x**2 + x + x)/x"
    ...
    def test_doesnt_simplify_sympy_by_default(self):
        ...
        assert preview["latex"] == "\\frac{x^{2} + x + x}{x}"
    ...
    def test_latex_with_equality_symbol(self):
        ...
        assert preview["sympy"] == "Eq((x**2 + x + x)/x, y)"
    ...
    def test_latex_conversion_preserves_default_symbols(self):
        ...
        assert preview.get("sympy") == "mu + x + 1"
    ...
    def test_latex_conversion_preserves_optional_symbols(self):
        ...
        assert preview["sympy"] == "m_table + test - x + 1"
    ...
    def test_sympy_conversion_preserves_optional_symbols(self):
        ...
        assert preview["latex"] == "m_{\\text{table}} + \\text{hello}_\\text{world} + x + 1"
